...
async function displayWeather(lat, lon, cityName) {
    ...
    applyWeatherBackground(lifestyleState);
...
function injectBackgroundCSS() {
    if (document.getElementById('weather-bg-css')) return;
    const style = document.createElement('style');
    style.id = 'weather-bg-css';
    style.textContent = `
        #weather-bg {
            position: fixed;
            inset: 0;
        ...
        /* ===== RAINY ===== */
        .bg-rainy {
            background: linear-gradient(180deg, #1a2332 0%, #0f1923 50%, #0d1520 100%);
        }
        .bg-rainy .rain-drops {
            position: absolute;
            inset: 0;
        ...
        .bg-rainy .rain-drop {
            position: absolute;
            width: 1.5px;
            background: linear-gradient(transparent, rgba(174, 194, 224, 0.45));
        ...
        .bg-rainy .rain-video.loaded {
            opacity: 0.35;
        }
        .bg-rainy .rain-overlay {
            position: absolute;
            inset: 0;
            background: linear-gradient(180deg, rgba(15,25,35,0.4) 0%, rgba(13,21,32,0.6) 100%);
        }

        /* ===== HOT ===== */
        .bg-hot {
            background: linear-gradient(180deg, #2a1a0e 0%, #1a0f08 50%, #120a05 100%);
        }
        .bg-hot .orb {
            position: absolute;
            border-radius: 50%;
        ...
        /* ===== COLD ===== */
        .bg-cold {
            background: linear-gradient(180deg, #0f1a2e 0%, #0a1020 50%, #080c18 100%);
        }
        .bg-cold .snowflake {
            position: absolute;
            background: white;
        ...
        @keyframes snowfall {
            0% { transform: translateY(-10vh) translateX(0); opacity: 0; }
            10% { opacity: var(--snow-opacity, 0.6); }
            90% { opacity: var(--snow-opacity, 0.6); }
            100% { transform: translateY(105vh) translateX(var(--snow-drift, 30px)); opacity: 0; }
        }
        .bg-cold .frost-glow {
            position: absolute;
            border-radius: 50%;
            filter: blur(100px);
            animation: frost-pulse ease-in-out infinite alternate;
        }
        .bg-cold .frost-glow-1 {
            width: 500px; height: 500px;
            background: radial-gradient(circle, rgba(100,160,255,0.08), transparent 70%);
            top: 10%; left: 20%;
            animation-duration: 8s;
        }
        .bg-cold .frost-glow-2 {
...
function createRainDrops(container) {
    ...
    dropsContainer.className = 'rain-drops';
    for (let i = 0; i < 60; i++) {
...
function createRainVideo(container) {
    const video = document.createElement('video');
    video.className = 'rain-video';
    video.muted = true;
    video.autoplay = true;
    video.loop = true;
    video.playsInline = true;
    video.src = 'https://videos.pexels.com/video-files/1918465/1918465-uhd_2560_1440_30fps.mp4';
...
function createSnowflakes(container) {
    for (let i = 0; i < 35; i++) {
        ...
        flake.style.cssText = `
            ...
            height: ${size}px;
            --snow-drift: ${drift}px;
            --snow-opacity: ${0.3 + Math.random() * 0.5};
...
function createHotOrbs(container) {
    ['orb-1', 'orb-2', 'orb-3', 'orb-4'].forEach(cls => {
...
function createColdGlows(container) {
    ['frost-glow-1', 'frost-glow-2'].forEach(cls => {
        const glow = document.createElement('div');
        glow.className = 'frost-glow ' + cls;
